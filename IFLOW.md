...
### 架构特点
- **模块化设计**：采用高度模块化的架构，各组件职责明确
- **异步渲染系统**：内置异步渲染管道，优化流式内容显示性能
- **性能监控**：内置性能监控工具，实时追踪渲染和API调用性能
- **缓存机制**：文件操作和渲染结果缓存，提升响应速度

## 技术栈

- **编程语言**: Go 1.25.5+
- **TUI 框架**: Bubble Tea v1.3.10 (charmbracelet/bubbletea)
- **UI 组件**: Bubbles v0.21.0 (charmbracelet/bubbles)
- **样式**: Lip Gloss v1.1.0 (charmbracelet/lipgloss)
- **配置管理**: YAML v3.0.1 (gopkg.in/yaml.v3)
- **Markdown 渲染**: Goldmark v1.7.13 (github.com/yuin/goldmark)
- **AI 模型**: GLM-4.5 (智谱AI)
...
├── internal/              # 内部包，不对外暴露
│   ├── api/              # GLM-4.5 API 客户端
│   │   ├── client.go     # API 客户端实现（支持流式响应和连接池）
│   │   └── types.go      # API 数据结构定义
│   ├── config/           # 配置管理
│   │   ├── config.go     # 配置文件读写
│   │   ├── config_test.go # 配置测试
│   │   └── tools_prompts.yaml.back # 工具提示配置备份
│   ├── mcp/              # Model Context Protocol 工具集成
│   │   ├── handler.go    # 工具处理器（支持工具注册表）
│   │   ├── protocol.go   # MCP 协议实现
│   │   ├── file_engine.go # 文件引擎（支持缓存和权限控制）
│   │   ├── file_tools.go # 文件工具
...
│   │   └── tool_test.go  # 工具测试
│   ├── tui/              # 终端用户界面（模块化架构）
│   │   ├── model.go      # Bubble Tea 主模型（协调器模式）
│   │   ├── model_state.go # 模型状态管理
│   │   ├── messages.go   # 消息处理逻辑
│   │   ├── messages_test.go # 消息处理测试
│   │   ├── async_renderer.go # 异步渲染器
│   │   ├── render_manager.go # 渲染管理器
│   │   ├── viewport_optimizer.go # 视口性能优化器
│   │   ├── stream_handler.go # 流式响应处理器
│   │   ├── command_processor.go # 命令处理器
...
### 模块化架构概览
PolyAgent 采用高度模块化的架构设计，将整个系统划分为多个独立但协作的模块：

```
┌─────────────────────────────────────────────────────────────┐
│                        主协调器 (Model)                      │
├─────────────────────────────────────────────────────────────┤
│  状态管理    │  渲染管理   │  流处理   │  命令处理   │  工具集成  │
│ (ModelState) │(RenderMgr) │(Stream)  │(Command)   │(Tools)    │
├─────────────────────────────────────────────────────────────┤
│                异步渲染管道 & 性能优化系统                    │
├─────────────────────────────────────────────────────────────┤
│  API客户端  │  MCP工具   │  配置管理  │  文件引擎   │  更新系统  │
└─────────────────────────────────────────────────────────────┘
```
...
- **主模型 (Model)**: 作为各模块的协调器，不直接处理业务逻辑
- **职责分离**: 每个组件有明确的单一职责
- **消息传递**: 通过 Bubble Tea 的消息系统进行组件间通信

#### 2. 异步渲染管道 (Async Rendering Pipeline)
- **渲染任务队列**: 优先级队列管理渲染任务
- **并发处理**: 多个渲染任务并发执行
- **结果缓存**: 渲染结果缓存避免重复计算
- **流式优化**: 专门优化流式内容的渲染性能

#### 3. 视口优化器 (Viewport Optimizer)
- **虚拟滚动**: 只渲染可见区域的内容
- **滚动缓冲**: 智能预加载和缓冲机制
- **性能节流**: 防止过度频繁的滚动更新
- **内存管理**: 智能内存使用和垃圾回收

#### 4. 状态管理系统 (State Management)
- **集中状态**: 所有应用状态集中在 ModelState
- **状态同步**: 确保各组件状态一致性
- **历史记录**: 支持状态历史和回滚
- **性能监控**: 状态变更的性能追踪
...
- **cmd/** 目录包含可执行程序的入口点
- 每个包有明确的单一职责
- **模块化架构**：TUI 采用协调器模式，各组件职责分离
- **接口优先**：关键组件定义接口，便于测试和扩展

### 架构模式
- **协调器模式**：主模型作为各模块协调器
- **管道模式**：异步渲染管道处理流式内容
- **注册表模式**：工具注册表管理可用工具
- **状态集中管理**：ModelState 集中管理应用状态
...
- **文件操作工具**: 安全的文件读写、目录遍历
- **文件创建工具**: 支持创建新文件和目录
- **网络搜索工具**: 集成 Tavily 搜索功能
- **网络爬取工具**: 集成 Tavily 网页爬取功能
- **代码分析工具**: 代码理解和分析
- **系统工具**: 环境信息获取
...
1. **MCP 工具系统**：完整的 Model Context Protocol 实现，包括工具注册表和动态提示生成
2. **Markdown 渲染**：内置 Markdown 渲染器支持，异步渲染优化性能
3. **工具集成管理**：统一的工具注册和调用机制
4. **命令解析系统**：增强的命令解析和处理，支持自然语言和斜杠命令
5. **编辑器集成**：与外部编辑器的集成支持
...
13. **异步渲染系统**：高性能异步渲染管道，优化流式内容显示
14. **视口优化器**：专门优化大文本滚动性能的组件
15. **性能监控**：内置性能监控工具，实时追踪渲染和API调用性能
16. **模块化架构**：采用协调器模式的模块化 TUI 架构
17. **文件引擎缓存**：文件操作缓存系统，减少重复访问
18. **HTTP 连接池**：网络连接复用，提升 API 调用性能
19. **CoT 思维链控制**：支持思维链模式的启用/禁用和历史查看
20. **配置文件引擎**：强大的文件引擎配置系统，支持路径白名单、文件类型黑名单等
...
8. **性能分析仪表板**：在 TUI 中显示详细的性能分析数据
9. **智能代码补全**：基于上下文的代码补全建议
10. **多模态输入**：支持图片和文档的上传与分析
